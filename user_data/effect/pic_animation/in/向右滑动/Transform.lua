...
local function funcEaseAction1(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.39, 0.575, 0.565, 1}
...
function Transform.new(construct, ...)
    ...
    self.material = nil
    self.duration = 0
    self.startPositionX = -2.0
    self.endPositionX = 0.0
    self.values = {}
    ...
    self.actions = 
    {
        -- 位移
        {
            startPosition = Amaz.Vector3f(self.startPositionX, 0.0, 0.0),
            endPosition = Amaz.Vector3f(self.endPositionX, 0.0, 0.0),
...
local function checkDirty(self)
    if self.tweenDirty then
        ...
        for i = 1, #self.actions do
            ...
            if action.startPosition ~= nil then
                target = transform
                action.startPosition.x = self.startPositionX * ratio
            ...
            if action.endPosition ~= nil then
                target = transform
                action.endPosition.x = self.endPositionX * ratio
